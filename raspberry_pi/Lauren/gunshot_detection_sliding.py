...
#uses sliding predictions, sliding each half a second
    #makes sure to not send out a text twice in a room (to avoid texting twice for the same gunshot)
...
# Multithreaded Inference: A callback thread adds microphone data to the audio analysis queue; The main thread, 
# an audio analysis thread, detects the presence of gunshot sounds in samples retrieved from the audio analysis 
# queue; And an SMS alert thread dispatches groups of messages to designated recipients.
...
def callback(in_data, frame_count, time_info, status):
    ...
    #put each second on the queue
    if len(sound_data) >= 44100:
...
detected_previous = False

# Get the first 2s long clip from the queue
all_mic_data = []
for i in range(0, 2):
    all_mic_data.append(np.array(audio_analysis_queue.get(), dtype = "float32"))    
...
while True:
    ...
    if maximum_frequency_value >= AUDIO_VOLUME_THRESHOLD:
        ...
        if probabilities_1[0][1] >= MODEL_CONFIDENCE_THRESHOLD and probabilities_2[0][1] >= MODEL_CONFIDENCE_THRESHOLD:
            # Sends out an SMS alert only if prev sliding analysis wasn't a gunshot
            if detected_prev == False:
    ...
    # Gets a new sample and its timestamp from the audio analysis queue
    microphone_data = microphone_data[44100:]
    microphone_data = np.append(microphone_data, np.array(audio_analysis_queue.get(), dtype = "float32"))
    time_of_sample_occurrence = audio_analysis_time_queue.get()
