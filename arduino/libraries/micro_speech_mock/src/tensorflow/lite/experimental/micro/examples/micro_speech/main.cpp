...
tflite::MicroInterpreter* newInterpreter = &interpreter;

FeatureProvider feature_provider(kFeatureElementCount,
                                 model_input -> data.uint8);
FeatureProvider* new_feature_provider = &feature_provider;

int32_t previous_time = 0;

RecognizeCommands recognizer(error_reporter);
RecognizeCommands* new_recognizer = &recognizer;
...
int loadModel() {
    ...
    SerialUSB.println("I made it here!");
...
int buildInterpreter() {
    // Build an interpreter to run the model with.
    newInterpreter = new tflite::MicroInterpreter(model, resolver, &tensor_allocator,
                                               error_reporter);

}

int requestMemoryInformation() {
    // Get information about the memory area to use for the model's input.
    model_input = (*newInterpreter).input(0);
...
int setupDataLoader() {
    ...
    new_feature_provider = new FeatureProvider(kFeatureElementCount, model_input -> data.uint8);
}

int setupCommandRecognizer() {
    new_recognizer = new RecognizeCommands(error_reporter);
}

// Keep reading and analysing audio data in an infinite loop.
int analyzeMicrophoneAudio() {
    ...
    TfLiteStatus invoke_status = (*newInterpreter).Invoke();
    ...
    TfLiteTensor* output = (*newInterpreter).output(0);
